# -*- coding: utf-8 -*-
"""
Created on Thu Jun  7 15:16:39 2018
...
def transform_pairwise(X, y):
    ...
    for k, (i, j) in enumerate(comb):
        ...
        if y_new[-1] != (-1) ** k:
            y_new[-1] = - y_new[-1]
            X_new[-1] = - X_new[-1]
    return np.squeeze(np.asarray(X_new)), np.squeeze(np.asarray(y_new).ravel())
...
if __name__ == '__main__':
    ...
    #separate out a test and training data set
    ...
    #this is the portion that saves the svm for later use
    from sklearn.externals import joblib
    joblib.dump(rank_svm, 'mysvm.pkl')
